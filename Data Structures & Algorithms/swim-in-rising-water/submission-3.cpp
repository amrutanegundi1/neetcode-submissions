...
public:
    ...
    int swimInWater(vector<vector<int>>& grid) {
        ...
        while(!que.empty())
        {
            ...
            que.pop();
            res = max(res, pr.first);
            addQueue(grid, que, visited, pr.second.first + 1, pr.second.second);
            addQueue(grid, que, visited, pr.second.first - 1, pr.second.second);
            addQueue(grid, que, visited, pr.second.first, pr.second.second + 1);
            addQueue(grid, que, visited, pr.second.first, pr.second.second - 1);
    ...
    void addQueue(vector<vector<int>>& grid, priority_queue<pair<int, pair<int,int>>, vector<pair<int, pair<int,int>>>, greater<pair<int, pair<int,int>>>> &que,set<pair<int, int>> &visited, int r, int c)
    {
