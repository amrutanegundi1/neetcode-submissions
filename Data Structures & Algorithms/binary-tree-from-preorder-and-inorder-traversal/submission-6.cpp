...
public:
    ...
    TreeNode* helper(vector<int>& preorder, vector<int>& inorder, int pl, int pr, int il, int ir) {
        if(pl < 0 || pl >= preorder.size() || pr < 0 || pr >= preorder.size()
         || il < 0 || il >= inorder.size() || ir < 0 || ir >= inorder.size() || pl > pr || il > ir)
        {
            return nullptr;
        }
        ...
        root->right = helper(preorder, inorder, pl + (ind - il) + 1, pr, ind + 1, ir); 
        return root;
    }
