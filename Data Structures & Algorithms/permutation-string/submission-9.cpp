    ...
    bool checkInclusion(string s1, string s2) {
        ...
        if (s2.length() >= s1.length()) {
            ...
            for (int i = 1; i <= s2.length() - window; i++) {
                dst[s2[i - 1]]--;
                if (dst[s2[i - 1]] != src[s2[i - 1]] && (dst[s2[i - 1]] + 1) == src[s2[i - 1]])
                    match--;
                else if(dst[s2[i - 1]] == src[s2[i - 1]])
                    match++;
                dst[s2[i + window - 1]]++;
                if (dst[s2[i + window - 1]] == src[s2[i + window - 1]])
                    match++;
                else if((dst[s2[i + window - 1]]-1) == src[s2[i + window - 1]])
                    match--;
                if (match == 26) {
                    return true;
                }
